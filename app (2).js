...
function init() {
  // —— 关键修复：启动顺序 ——
  // 脚本顶层在“图片层(IndexedDB)就绪之前”就读取过一次本地数据，
  // 那时图片令牌(idb::img::…)还无法还原成照片，导致重启后壁纸、
  // 笔记配图、法相彩页等看似“没保存上”。init 由图片层 boot 之后调用，
  // 此刻重新读取一遍，所有照片即可正确还原。
  try { state.notes = loadNotes(); } catch (e) { console.warn("reload notes failed", e); }
  try { state.teachingQuotes = loadTeachingQuotes(); } catch (e) { console.warn("reload teachings failed", e); }
  try { state.modules = loadModules(); } catch (e) { console.warn("reload modules failed", e); }
  try {
    pageWallpapers = loadPageWallpapers();
    state.pageWallpapers = pageWallpapers;
  } catch (e) { console.warn("reload wallpapers failed", e); }
